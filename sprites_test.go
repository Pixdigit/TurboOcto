...
func TestSpriteLayers(t *testing.T) {
	...
	sp, err := NewSprite()
	if err != nil {
		wrapErr(err, "could not create test sprite", t)
	}
	sp2, err := NewSprite()
	if err != nil {
		wrapErr(err, "could not create test sprite", t)
...
func TestSpriteRendering(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		if err != nil {
			wrapErr(err, "error while creating test textures", t)
		}
	...
	if err != nil {
		wrapErr(err, "error while setting configuration", t)
	}
	...
	for i := int32(0); i < dFrames; i++ {
		...
		err = sp.Blit()
		if err != nil {
			wrapErr(err, "could not blit sprite", t)
		}
		Present()
	}
