...
func initializeEnvironment() (err error) {
	...
	ok, err := pathExists("./conf")
	if err != nil {
		return errors.Wrap(err, "could not check for configuration")
	}
	if ok {
		ok, err := pathExists("./conf/last.csv")
		if err != nil {
			return errors.Wrap(err, "failed to check for last configuration")
		}
		if ok {
			err := LoadConf("last")
			if err != nil {
				return errors.Wrap(err, "could not load last configuration")
			}
		} else {
			err := LoadDefaultConf()
			if err != nil {
				return errors.Wrap(err, "could not initialize environment")
			}
		}
	} else {
		err := LoadDefaultConf()
...
func LoadDefaultConf() error {
	ok, err := pathExists("./conf/default.csv")
	if err != nil {
		return errors.Wrap(err, "failed to check for default configuration")
	}
	if ok {
		err := LoadConf("default")
		if err != nil {
			return errors.Wrap(err, "could not load default configuration")
		}
	} else {
		//Default Configuration
		err := AddConf("updateOnRefresh", true)
		if err != nil {
			return errors.Wrap(err, "could not set default configuration")
		}
		err = AddConf("fullscreen", true)
		if err != nil {
			return errors.Wrap(err, "could not set default configuration")
		}
		err = AddConf("allowFrameSkipping", true)
		if err != nil {
			return errors.Wrap(err, "could not set default configuration")
		}
		err = AddConf("spriteTimerMode", USE_FRAME_COUNT)
		if err != nil {
			return errors.Wrap(err, "could not set default configuration")
		}
	}
...
func deserialize(raw string) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "could not deserialize \""+raw+"\"")
	}
...
func GetConf(confName string) (interface{}, error) {
	...
	if err != nil {
		return nil, errors.New("invalid configuration value \"" + config + "\"")
	}
	return value, nil
}
...
func SetConf(confName string, confValue interface{}) error {
	...
	if err != nil {
		return errors.Wrap(err, "could not change configuration")
	}
	oldVarType, err := typeOfSerialized(config)
	if err != nil {
		return errors.Wrap(err, "could not check var type")
	}
	newVarType, err := typeOfSerialized(config)
	if err != nil {
		return errors.Wrap(err, "could not check var type")
	}
...
func AddConf(confName string, initConfValue interface{}) error {
	...
	if err != nil {
		return errors.Wrap(err, "could not serialize initial conf value")
	}
	conf[confName] = newConfig
	return nil
}
...
func SaveConf(filename string) error {
	...
	if !ok {
		...
		if err != nil {
			return errors.Wrap(err, "could not create conf folder")
		}
	}
	...
	defer file.Close()
	if err != nil {
		return errors.Wrap(err, "could not open file "+filename+" to save configuration")
	}
	w := csv.NewWriter(file)
	err = w.WriteAll(data)
	if err != nil {
		return errors.Wrap(err, "could not write configuration to file"+filename)
	}
...
func LoadConf(filename string) error {
	...
	defer file.Close()
	if err != nil {
		return errors.Wrap(err, "could not open file "+filename+" to load configuration")
	}
	r := csv.NewReader(file)
	r.FieldsPerRecord = 2
	data, err := r.ReadAll()
	if err != nil {
		return errors.Wrap(err, "could not read configuration from file "+filename)
	}

	for _, configuration := range data {
		//test if value can be deserialized == valid value
		_, err := deserialize(configuration[1])
		if err != nil {
			return errors.Wrap(err, "could not load conf file \""+filename+"\"")
		}
